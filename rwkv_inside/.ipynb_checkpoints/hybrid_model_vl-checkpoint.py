...
if os.environ["architecture"] == 'hxa079':
    from TimeMixer_vl import RWKV_Tmix_x070_Mose_cxa079 as TimeMixer
elif os.environ["architecture"] == 'hxa07a':
    from TimeMixer_vl import RWKV_Tmix_x070_Mose_cxa07A as TimeMixer
elif os.environ["architecture"] == 'hxa07b':
    from TimeMixer_vl import RWKV_Tmix_x070_Mose_cxa07B as TimeMixer
#from PaTHAttention import PaTHAttention as SelfAttention
from TimeMixer_vl import GQAWithRopeAttention as SelfAttention
...
class HybridModel(nn.Module):
    ...
    def get_rwkv_args(transformer_config):
        ...
        args.ctx_len = 4096
        args.n_layer = transformer_config.text_config.num_hidden_layers
        args.n_embd = transformer_config.text_config.hidden_size
        args.dim_att = transformer_config.text_config.hidden_size
        args.dim_ffn = transformer_config.text_config.intermediate_size
        args.pre_ffn = 0
        args.head_qk = 0
        args.tiny_att_dim = 0
        args.tiny_att_layer = -999
        args.vocab_size = transformer_config.text_config.vocab_size
        args.layers = [i for i in range(transformer_config.text_config.num_hidden_layers)]
        args.pad_id = transformer_config.text_config.eos_token_id
        args.stage = 4
    ...
    def __init__(self, transformer_model, rwkv_args, tokenizer=None):
        ...
        self.model.model.language_model.get_input_embeddings().register_forward_hook(embedding_hook)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for layer_idx in range(self.model.config.text_config.num_hidden_layers):
            if layer_idx in self.args.layers:
                llama_layer = self.model.model.language_model.layers[layer_idx]
