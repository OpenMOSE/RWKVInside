...
class RWKV_Tmix_x070_Mose_cxa079(torch.nn.Module):
    def __init__(self, args, layer_id):
        ...
        self.max_position_embeddings = args.config.text_config.max_position_embeddings
        self.num_attention_heads = args.num_attention_heads
        self.num_key_value_heads = args.num_key_value_heads
        self.num_key_value_groups = self.num_attention_heads // self.num_key_value_heads
        self.rms_norm_eps = args.rms_norm_eps
        self.rope_theta = args.config.text_config.rope_theta
...
class RWKV_Tmix_x070_Mose_cxa07A(torch.nn.Module):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
        ...
        with torch.no_grad():
            ...
            def ortho_init_gpu_then_cpu(x, scale=1.0):
                """
                高速Orthogonal初期化:
            ...
            #07A Default
            D_DECAY_LORA = max(32, int(round(  (1.7*(C**0.6))  /32)*32))# suggestion
            D_AAA_LORA   = max(32, int(round(  (1.3*(C**0.5))  /32)*32))
            D_GATE_LORA  = max(32, int(round(  (1.7*(C**0.6))  /32)*32))# suggestion

            
            print(f'D_DECAY_LORA={D_DECAY_LORA}')
            self.w1 = nn.Parameter(torch.zeros(Hidden_dim, D_DECAY_LORA))
            self.w2 = nn.Parameter(ortho_init_gpu_then_cpu(torch.zeros(D_DECAY_LORA, self.num_attention_heads * self.head_size), 0.1))
            self.w0 = nn.Parameter(www.reshape(1,1,self.num_attention_heads * self.head_size) + 0.5 + zigzag*2.5)
            
            print(f'D_AAA_LORA={D_AAA_LORA}')
            self.a1 = nn.Parameter(torch.zeros(Hidden_dim, D_AAA_LORA))
            self.a2 = nn.Parameter(ortho_init_gpu_then_cpu(torch.zeros(D_AAA_LORA, self.num_attention_heads * self.head_size), 0.1))
            self.a0 = nn.Parameter(torch.zeros(1,1,self.num_attention_heads * self.head_size)-0.19 + zigzag*0.3 + linear*0.4)
            #self.r_k = nn.Parameter(torch.zeros(H,N)-0.04)

            
            self.g1 = nn.Parameter(torch.zeros(Hidden_dim, D_GATE_LORA))
            self.g2 = nn.Parameter(ortho_init_gpu_then_cpu(torch.zeros(D_GATE_LORA, C), 0.1))
            print(f'D_GATE_LORA={D_GATE_LORA}')
    ...
    def forward(self, x, v_first,k_first,attention_mask,position_embeddings,position_ids,x_emb): 
        ...
        v = repeat_kv(v, self.num_key_value_groups).view(B, T, -1)
...
class RWKV_Tmix_x070_Mose_cxa07B(torch.nn.Module):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
        ...
        with torch.no_grad():
            ...
            D_DECAY_LORA = max(256, int(round(  (1.7*(C**0.6))  /32)*32))# suggestion
            D_AAA_LORA   = max(128, int(round(  (1.8*(C**0.5))  /32)*32)) #suggestion
            D_GATE_LORA  = max(256, int(round(  (1.7*(C**0.6))  /32)*32))# suggestion
            D_MK_LORA = max(32, int(round(  (1.3*(C**0.5))  /32)*32)) #Modified
            D_MV_LORA = max(32, int(round(  (1.3*(C**0.5))  /32)*32)) #Modified
    ...
    def forward(self, x, v_first,k_first,attention_mask,position_embeddings,position_ids,x_emb): 
        ...
        w = -F.softplus(-(self.w0 + F.tanh(x @ self.w1) @ self.w2)) -0.5
