...
def compute_kl_loss_(student_outputs, teacher_logits, labels, args, attention_mask=None, chunk_size=4096, topk=10000):
    ...
    if args.enable_AKL:
        # For Adaptive KL loss implementation remains the same
        kl_loss = compute_adaptive_kl_loss(
            student_logits, 
            teacher_logits,
            attention_mask=attention_mask
        ...
        # Get TopK values and indices from teacher logits
        with torch.no_grad():
            teacher_topk_values, teacher_topk_indices = torch.topk(teacher_logits, topk, dim=-1)
            
            # Create targets with only topk probabilities
            targets_sparse = F.softmax(teacher_topk_values, dim=-1)
        
        # Compute log probs only for the selected indices from student model
        batch_size, seq_len = student_logits.shape[:2]
        
        # Create batch indices and seq indices for gathering
        batch_indices = torch.arange(batch_size).view(-1, 1, 1).expand(-1, seq_len, topk).to(student_logits.device)
        seq_indices = torch.arange(seq_len).view(1, -1, 1).expand(batch_size, -1, topk).to(student_logits.device)
        
        # Gather student logits at teacher's topk indices
        gathered_student_logits = student_logits[batch_indices, seq_indices, teacher_topk_indices]
        
        # Compute softmax over just these topk logits
        log_probs_student_topk = F.log_softmax(gathered_student_logits, dim=-1)
        
        # Compute KL divergence without reduction
        kl_div_all = F.kl_div(
            log_probs_student_topk,
            targets_sparse,
            reduction='none'
        ...
        if attention_mask is not None:
            ...
            masked_kl = kl_div_per_token * attention_mask
            kl_loss = masked_kl.sum() / (attention_mask.sum() + 1e-6)
        else:
            kl_loss = kl_div_per_token.mean()
        
        # Clean up memory
        del log_probs_student_topk, targets_sparse, kl_div_all, kl_div_per_token
        del batch_indices, seq_indices, teacher_topk_values, teacher_topk_indices
